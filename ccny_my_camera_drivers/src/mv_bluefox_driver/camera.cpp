...
//  pnode.param("device", device);exposition_time_sub = nh_.subscribe ( exp_time_topic, 1, &ImageConverter::save_ti

  pnode.param("use_color", use_color_, false);
  pnode.param("fps", fps, 30);
  pnode.param("skip_frames", skip_frames, 0);
  pnode.param("expose_us", expose_us_, expositionTime);
  frames_to_skip = 0;
...
//   pnode.param("height", height_, 480);
  ...
  pnode.getParam("num_camera", num_camera);
...
// 
//   control.pid_update(error, expositionTime);
//   expositionTime =  expositionTime + control.control;
//  //expositionTime =  expositionTime + coeff*error;
//      
//   //std::cout<<" the expositionTime is " << expositionTime << std::endl;
//   expositionTime = std::max(expositionTime,12);
...
/*
void Camera::control_exposition(sensor_msgs::ImagePtr image, float &error)
{
  float pixel_value;
  int i,j,n,x=0,m=0,p=0;

    scanf("%d",&n); /*READING ELEMENTS*/
//     for(i=0;i<n;i++)
//     {
//         for(j=0;j<n;j++)
//         scanf("%d",&a[i][j]);
...
//   
//   
//   pixel_value=cv::sum(image)(0)/(480*752);
//   //pixel_value = 180;
//   error= 128-pixel_value;*/
// }
